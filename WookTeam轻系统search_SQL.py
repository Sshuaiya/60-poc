...
def banner():
    test = """WookTeam轻量级的团队在线协作系统接口searchinfo存在SQL注入漏洞"""
    print(test)
def poc(target):
    ...
    try:
        ...
        if response.status_code == 200 and "username" in response.text:
            ...
            print(f"{target}可能存在sql注入请手工测试")
def main():
    banner()
    url_list = []
    parse = argparse.ArgumentParser(description="WookTeam轻量级的团队在线协作系统接口searchinfo存在SQL注入漏洞")
